    ...
    for leaf_module_name, leaf_module in leaf_modules.items():
        ...
        for i in range(len(names)):
            ...
            if i == len(names) - 1:  # leaf module itself
                ...
                node.inference_memory = leaf_module.inference_memory.cpu().numpy()[0]
                node.MAdd = leaf_module.MAdd.cpu().numpy()[0]
...
class ModelStat(object):
    ...
    def _analyze_model(self):
        model_hook = ModelHook(self._model, self.data , self._input_size)
        leaf_modules = model_hook.retrieve_leaf_modules() #model layers 
...
def stat(model, data, input_size, query_granularity=1):
    ms = ModelStat(model, data, input_size, query_granularity)
    ms.show_report()
